...
//			MAFIA TARGET
			...
			if (mainpage.all_players.contains(user)) {
				String selection;
				while (true) {
					System.out.print("                          MAFIA PLEASE SELECT PLAYER FOR TARGET : ");
					selection = (scn.next()).toUpperCase();
					if (!player_present(selection)) {
						System.out.println("                           THE PLAYER IS NOT PRESENT OR THE NAME IS WRONG");
						...
						if (tf) {
							...
							System.out.println(
									"                           PLEASE ENTER ANYOTHER PLAYER THISONE IS MAFIA");
						}
					}
				}
				for (int i = 0; i < mainpage.all_players.size(); i++) {
					if (mainpage.all_players.get(i).getName().equals(selection)) {
						double random_hp = mainpage.all_players.get(i).getHp();
						if (random_hp > combined_hp) {
							mainpage.all_players.get(i).setHp(random_hp - combined_hp);
						} else {
							mainpage.all_players.get(i).setHp(0);
						}
						break;
					}
				}
...
//					MAFIAS VOTE
					if (mainpage.all_players.contains(user)) {
						...
						String selection;
						while (true) {
							System.out.print("                          MAFIA PLEASE SELECT PLAYER FOR VOTING : ");
							selection = (scn.next()).toUpperCase();
							if (!player_present(selection)) {
								System.out.println(
