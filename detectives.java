...
class detectives extends player {
	detectives(String NAME) {
		super(NAME, "DETECTIVE");
	}
...
//			DETECTIVE TEST
			...
			if (mainpage.all_detectives.size() > 0) {
				if (mainpage.all_players.contains(user)) {
					String selection;
					while (true) {
						System.out.print("                         DETECTIVE PLEASE SELECT PLAYER FOR TEST : ");
						...
						if (!player_present(selection)) {
							...
							for (int i = 0; i < mainpage.all_players.size(); i++) {
								if (mainpage.all_players.get(i).getName().equals(selection)) {
									if (mainpage.all_players.get(i).getType().equals("DETECTIVE")) {
										tf = false;
										break;
									}
								}
							...
							if (tf) {
								...
								System.out.println(
										"                       PLEASE ENTER ANYOTHER PLAYER THISONE IS DETECTIVE");
							}
						}
					}
					player_for_test = selection;
					for (int i = 0; i < mainpage.all_players.size(); i++) {
						if (mainpage.all_players.get(i).getName().equals(selection)) {
							if (mainpage.all_players.get(i).getType().equals("MAFIA")) {
								System.out.println("                                        "
										+ mainpage.all_mafias.get(i).getName() + " IS A MAFIA");
								default_vote = true;
								break;
							}
						}
...
//					DETECTIVE VOTE
					if (mainpage.all_detectives.size() > 0) {
						if (mainpage.all_players.contains(user)) {
							...
							while (true) {
								System.out
										.print("                       DETECTIVE PLEASE SELECT PLAYER FOR VOTEOUT : ");
								selection = (scn.next()).toUpperCase();
								if (!player_present(selection)) {
									System.out.println(
											"                           THE PLAYER IS NOT PRESENT OR THE NAME IS WRONG");
							...
							System.out.println("                               DETECTIVE CHOOSED PLAYERS TO VOTE");
						}
					}
//					COMMONER VOTE
					if (mainpage.all_commoners.size() > 0) {
						...
						System.out.println("                                   COMMONER CHOOSED PLAYERS TO VOTE");
					}
//						VOTE TIE
